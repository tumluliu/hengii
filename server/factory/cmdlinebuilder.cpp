...
CmdlineBuilder::CmdlineBuilder(const MetaRepo &repo) : repo_(repo) {
}

int CmdlineBuilder::Build(string &result, const Job &job) const {
	result = "";
	string cache = "";
...
int CmdlineBuilder::BuildAppOptions(
		...
		const std::string &appuri) const {
	result = "";
	std::vector<AppOption> meta;

	repo_.BuildMeta(meta, appuri);
...
int CmdlineBuilder::BuildLoader(string &result, Context env) const {
