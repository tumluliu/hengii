...
Validator::Validator(const JobValidator &sub)
	: nextstage_(NULL), substage_(&sub) {
	}

Validator::Validator(const Validator *next, const JobValidator &sub)
...
bool Validator::Chanllenge(const hpgc::higis::interface::JobFlow &flow,
	...
	if (!LetPass(flow, message)) {
		...
		return nextstage_->LetPass(flow, message);
	}
	return true;
}
