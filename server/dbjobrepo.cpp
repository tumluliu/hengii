...
void DbJobRepo::UpdateJobFlowRuntime(const FlowRuntime *data) {
	...
	for (int i = 0; i < data->get_jobcount(); i++) {
		JobRuntime *one = data->get_jobruntime(i);
		UpdateJobRuntime(one);
	}
}
...
bool DbJobRepo::HasConflictFlowId(int64_t id) const {
	std::vector<string> conflict;
	db_->QueryRow(conflict, GetConflictFlowIdSql(id));
	if (conflict[0] == "0") {
		return false;
	}
	else {
...
FlowRuntime *DbJobRepo::BorrowJobFlowRuntime(int64_t id) { 
	FlowRuntime *fp = new FlowRuntime(id);
	std::string msg;
	...
	for (std::vector<int>::const_iterator i = jobids.begin();
			i != jobids.end(); i++) {
		JobRuntime *jp = new JobRuntime(id, *i);
		std::string jobmsg;
...
void DbJobRepo::ReturnJobFlowRuntime(FlowRuntime *fp) {
	if (fp != NULL) {
		delete fp;
	}
	else {
		Log().Error() << "I'm job repo, You give me a empty runtime...";
	}
}
