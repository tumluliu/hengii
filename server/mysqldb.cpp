...
MYSQL_RES *MySqlDb::Query( const std::string &query ) {
	int ret;
	MYSQL *conn;
	MYSQL_RES *res;
	int ping;
...
MYSQL *MySqlDb::BorrowConnection() {
	unsigned int i;
...
int MySqlDb::ReturnConnection(MYSQL *conn) {
	unsigned int i;
	bool known = false;
...
void MySqlDb::QueryColumn(std::vector<string> &result, const string &sql) {
	MYSQL_RES *res;
	MYSQL_ROW r = NULL;
...
void MySqlDb::QueryRow(std::vector<string> &result, const string &sql) {
	MYSQL_RES *res;
	MYSQL_ROW r = NULL;
